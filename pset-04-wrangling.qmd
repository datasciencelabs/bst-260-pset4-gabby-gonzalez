...
population1<-resp_body_json(response, simplifyVector = TRUE)
...
population<-population1 |>
    ...
    rename(state_name=NAME,  # rename NAME column to state_name
           "2020"=POP_2020, # remove POP_ from year
           "2021"=POP_2021) |> # remove POP_ from year
   mutate(state = case_when(   # use case_when to add abbreviations for DC and PR
    state_name == "District of Columbia" ~ "DC",
    state_name == "Puerto Rico" ~ "PR",
    TRUE ~ state.abb[match(state_name, state.name)] # add state abbreviations using state.abb variable
  )) |>
  pivot_longer(      #create a year column.
  cols = c("2020", "2021"),
  names_to = "year",
  values_to = "population"
) |>
  arrange(state_name) |> #arrange states in alphabetical order
  mutate(population=as.numeric(population)) #make pop var numeric
...
population |>
  ggplot(aes(x = reorder(state_name, population), y = population, fill = year)) +
  geom_col() +
  coord_flip() +
  facet_wrap(~year) +
  labs(x = "State", y = "Population", title = "Population") +
    theme_classic()
...
response.10 <- request(api) |> req_perform() |> resp_body_json(simplifyVector = TRUE)
...
All of the data is not there. This is because the API limits the number of entries that we are able to download. In this case, the limit is only 1000 entries.
...
cases_raw |> 
  ...
  right_join(population, by = c("state", "year")) |>
  mutate(cases_per=100000*(cases/population)) |>
  ggplot(aes(x=date, y=cases_per, color=state)) +
  geom_line() +
  labs(title = "Time Series Plot for Covid-19 Cases per 100,000", y = "Cases per 100,000", x="Date", color = "State Abbreviation")+
  theme_linedraw()+
...
cases_raw |>
  #I already changed date from a character string in an earlier step
  ...
  arrange(year, month) |>
  filter(year %in% c(2020, 2021)) |>
  kable(col.names = c("Year","Month", "Total Covid Cases"))
...
```{r}
deaths<-request(deaths_url) |> req_url_query(`$limit` = 10000000000) |>
  req_perform() |> 
  resp_body_json(simplifyVector = TRUE) |>
  select(state, covid_19_deaths, end_date) |>
  rename(deaths=covid_19_deaths, date=end_date) |>
...
deaths |>
  ...
  labs(title = "Top 10 Highest Covid Deaths by State", y = "Total Deaths (2020-2023)", x="State")+
  theme_linedraw()+
  theme(axis.title = element_text(hjust = 0.5, face = "bold"), plot.title = element_text(hjust = 0.5, face = "bold"))
