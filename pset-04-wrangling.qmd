...
regions<-fromJSON(url) |>
  ...
  unnest(state_name) ##WRONG FUNCTION NEED TO USE map_df
?fromJSON
?map_df
regions
...
population2 <- population |>
  mutate(region=regions[match(state_name,regions$state_name)])
...
#dates are already in ISO-8601 format
...
# need to join with population dataset -- then get region -- then get cases/100,000

?str_detect
cases_raw |> 
  mutate(cases_per=cases/100,000) |>
  # str_subset(date, "2023-") |>
  ggplot(aes(x=date, y=cases_per, group=state)) +
  geom_line() 
  facet_wrap(state)
...
```{r}
cases_raw |> 
  tibble(state=state, cases=cases, date = date, month = month(date), year = year(date))
